...
export class AuroraComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    container.appendChild(gl.canvas);
      ...
      const update = (t: number) => {
        ...
        const time = (t - this.startTime) * 0.01;
        if (this.program) {
          ...
          this.program.uniforms['uColorStops'].value = stops.map((hex: string) => {
            const c = new Color(hex);
            return [c.r, c.g, c.b];
          });
  ...
  ngOnDestroy() {
    ...
    if (this.renderer) {
      const gl = this.renderer.gl;
      if (this.containerRef?.nativeElement && gl.canvas.parentNode === this.containerRef.nativeElement) {
        this.containerRef.nativeElement.removeChild(gl.canvas);
      }
