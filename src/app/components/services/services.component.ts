...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, LucideAngularModule, TypewriterDirective],
...
export class ServicesComponent implements OnInit {
  ...
  ngOnInit() {
    // Initialize selected currency from global preference
  ...
  loadServices() {
    ...
    this.dataService.getServices().subscribe({
      next: (dbServices: Service[]) => {
        ...
        // Show only 2 random services on home page
        const active = dbServices.filter(service => service.status === 'active');
        // Shuffle and take 2
        const shuffled = active.sort(() => Math.random() - 0.5);
        this.services = shuffled.slice(0, 2)
          .map(service => ({
            id: service.id,
            title: service.title,
      ...
      error: (err) => {
        ...
        this.loading = false;
        // Keep empty array on error - won't show anything
        this.services = [];
      }
    });
  }
